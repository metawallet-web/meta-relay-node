          ...
          if(firstQueuedMetaPacket.type == 'lava')
          {
            ...
            var txResult = await LavaPacketSubmitter.broadcastLavaPacket(
              ...
              if(txResult.success == true){
                ...
                result =  await RelayHelper.setStatusOfLavaPacket( firstQueuedMetaPacket, 'pending' , this.mongoInterface )
       

              }else{ 
 
                let result =  await RelayHelper.setStatusOfLavaPacket( firstQueuedMetaPacket, 'reverted' , this.mongoInterface )
...
/*
  static async sendSignedRawTransaction(web3,txOptions,addressFrom,private_key,callback) {
    ...
    const transaction = new EthereumTx(txOptions)
