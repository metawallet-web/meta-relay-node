...
module.exports = class LavaPacketSubmitter  {
  ...
  async pollLavaPacketsToSubmit()
  {
   ...
   async processQueuedPacket(firstQueuedMetaPacket){
    ...
    if(firstQueuedMetaPacket.type == 'lava')
    {
      ...
      var txResult = await LavaPacketSubmitter.broadcastLavaPacket(
        ...
        console.log('txResult', txResult)
          ...
          result =  await RelayHelper.setStatusOfPacket( firstQueuedMetaPacket, 'pending' , this.mongoInterface )
  ...
  async monitorPendingLavaPackets()
  {
    ...
    if( pendingPacket )
    {
      ...
      if( !txData ){
        ...
        let result =  await RelayHelper.setStatusOfPacket( pendingPacket, 'queued' , this.mongoInterface )
  ...
  static async broadcastLavaPacket(packetData,relayingGasPrice,relayAccount,web3)
  {
    ...
    return await LavaPacketSubmitter.broadcastUsingTxMethod( web3, lavaTransferMethod, relayingGasPrice, addressFrom, addressTo , pKey)   
  ...
  static async broadcastUsingTxMethod(web3, txMethod, relayingGasPrice, addressFrom, addressTo, pKey){
     ...
     */
       ...
       let encoded_tx = txMethod.encodeABI();
