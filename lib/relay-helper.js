...
module.exports= class RelayHelper{
    ...
    static async storeNewLavaPacket(lavaPacket, chainId, mongoInterface){
        ...
        if(packetIsValid){
            ...
            let newPacket = {
                ...
                status: 'queued',
                fromAddress: web3utils.toChecksumAddress( lavaPacket.from ) ,
                type: 'lava',
                txdata: null,
    ...
    static async storeNewPermitPacket(permitPacket, chainId, mongoInterface){
        ...
        if(packetIsValid){
            ...
            let newPacket = {
                ...
                fromAddress: web3utils.toChecksumAddress( permitPacket.from ) ,
                status: 'queued',
                type: 'permit',
                birthtime: Math.round(+new Date()/1000) 
    ...
    static async findRecentPacketsFromAddress(publicAddress, chainId, mongoInterface){

       return  await mongoInterface.findAllSortedWithLimit('metapackets', {"fromAddress": web3utils.toChecksumAddress(publicAddress) } , {birthtime:-1}, 50  )
